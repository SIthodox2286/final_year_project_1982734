...
timepoints = [2071,3511,4951,6391,7831,7831+1440,7831+2*1440,7831+3*1440,...
    7831+4*1440,7831+5*1440,7831+6*1440,7831+7*1440,7831+8*1440,7831+9*1440,...
    7831+10*1440,7831+11*1440];
%

mismatch_sets = zeros(length(timepoints),13);

%
R_frequencies = baseline_gapped.R_Frequency;
Y_frequencies = baseline_gapped.Y_Frequency;
B_frequencies = baseline_gapped.B_Frequency;
...
beginTime11 = timepoints(2);
endTime11 = beginTime11+1440-1;
endTime1 = beginTime1+1440-1;
beginTime2 = timepoints(3);
endTime2 = beginTime2+1440-1;
beginTime3 = timepoints(5);
beginTime33 = timepoints(4)-100;
endTime33 = beginTime33+1440-1;
...
hold off;
legend('B-Frequency, the base','R-Frequency Window','Windowed B-Frequency')
...
plot(time(1:endTime3+100),B_frequencies(1:endTime3+100),'b.');
plot(time(beginTime11:endTime11),R_frequencies(beginTime2:endTime2),'ro')
plot(time(beginTime11:endTime11),B_frequencies(beginTime1:endTime1),'gx')
hold off;
legend('B-Frequency, the base','R-Frequency Window','Windowed B-Frequency')
...
plot(time(1:endTime3+100),B_frequencies(1:endTime3+100),'b.');
plot(time(beginTime33:endTime33),R_frequencies(beginTime2:endTime2),'ro')
plot(time(beginTime33:endTime33),B_frequencies(beginTime33:endTime33),'gx')
legend('B-Frequency, the base','R-Frequency Window','Windowed B-Frequency')
...
for i = 1:size(timepoints)
    ...
    for d = -1440:1440
        M = corrcoef(R_frequencies(beginTime:endTime),Y_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end

    differences_RY = [differences',value'];
    mismatch_RY = value(find(differences==max(differences)));
    mismatch_RY = max(mismatch_RY); % mismatch_i, not all the values
    %
    differences = zeros(1,2880);
    value = -1440:1440;
    for d = -1440:1440
        M = corrcoef(R_frequencies(beginTime:endTime),B_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_RB = [differences',value'];
    mismatch_RB = value(find(differences==max(differences)));
    mismatch_RB = max(mismatch_RB);

    differences = zeros(1,2880);
    value = -1440-mismatch_RY:1440-mismatch_RY;
    for d = -1440:1440
        M=corrcoef(Y_frequencies(beginTime+mismatch_RY:endTime+mismatch_RY),B_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_YB = [differences',value'];
    mismatch_YB = value(find(differences==max(differences)));
    mismatch_YB = max(mismatch_YB);

    differences = zeros(1,2880);
    value = -1440-mismatch_RB:1440-mismatch_RB;
    for d = -1440:1440
        M = corrcoef(B_frequencies(beginTime+mismatch_RY:endTime+mismatch_RY),Y_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_BY = [differences',value'];
    mismatch_BY = value(find(differences==max(differences)));
    mismatch_BY = max(mismatch_BY);
    ...
    value = -1440:1440;
    for d = -1440:1440
        M = corrcoef(Y_frequencies(beginTime:endTime),R_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
   
    differences_YR = [differences',value'];
    mismatch_YR = value(find(differences==max(differences)));
    mismatch_YR = max(mismatch_YR);
    
    % SlidingYonB
    differences = zeros(1,2880);
    value = -1440:1440;
    for d = -1440:1440
        M = corrcoef(Y_frequencies(beginTime:endTime),B_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_YB2 = [differences',value'];
    mismatch_YB2 = value(find(differences==max(differences)));
    mismatch_YB2 = max(mismatch_YB2);
    ...
    for d = -1440:1440
        M = corrcoef(R_frequencies(beginTime+mismatch_YR:endTime+mismatch_YR),B_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_RB2 = [differences',value'];
    mismatch_RB2 = value(find(differences==max(differences)));
    mismatch_RB2 = max(mismatch_RB2);
    ...
    for d = -1440:1440
        M = corrcoef(B_frequencies(beginTime+mismatch_YR:endTime+mismatch_YR),R_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_BR = [differences',value'];
    mismatch_BR = value(find(differences==max(differences)));
    mismatch_BR = max(mismatch_BR);
    ...
    value = -1440:1440;
    for d = -1440:1440
        M = corrcoef(B_frequencies(beginTime:endTime),R_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end

    differences_BR2 = [differences',value'];
    mismatch_BR2 = value(find(differences==max(differences)));
    mismatch_BR2 = max(mismatch_BR2);
    
    % SlidingBonY
    differences = zeros(1,2880);
    value = -1440:1440;
    for d = -1440:1440
        M = corrcoef(B_frequencies(beginTime:endTime),Y_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_BY2 = [differences',value'];
    mismatch_BY2 = value(find(differences==max(differences)));
    mismatch_BY2 = max(mismatch_BY2);
    ...
    for d = -1440:1440
        M = corrcoef(R_frequencies(beginTime+mismatch_BR2:endTime+mismatch_BR2),Y_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_RY2 = [differences',value'];
    mismatch_RY2 = value(find(differences==max(differences)));
    mismatch_RY2 = max(mismatch_RY2);
    ...
    for d = -1440:1440
        M = corrcoef(Y_frequencies(beginTime+mismatch_BR2:endTime+mismatch_BR2),R_frequencies((beginTime+d):(endTime+d)));
        differences(d+1441) = M(1,2);
    end
    
    differences_YR2 = [differences',value'];
    mismatch_YR2 = value(find(differences==max(differences)));
    mismatch_YR2 = max(mismatch_YR2);
...
%%
mismatchTable_PCC_Baseline = mismatchTable;
save('dataIncluded\PCC_Baseline_mismatch.mat','mismatchTable_PCC_Baseline');
...
xlabel('Sequences')
%%
overallSequencing_Coff_transition=overallSequencing;
save('overallSequencing_Coff_transition.mat','overallSequencing_Coff_transition');
